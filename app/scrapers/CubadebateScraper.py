...
class CubadebateScraper(BaseScraper):
    ...
    def __init__(self, categoria: str):
        ...
        if categoria not in dataNews:
            raise ValueError(f"Categoría '{categoria}' no encontrada. Opciones: {list(dataNews.keys())}")
    ...
    def scrape(self) -> List[Dict]:
        ...
        try:
            ...
            articulos = soup.find_all('div', class_=lambda x: x and clase_buscar in x.split())
            ...
            if not articulos:
                ...
                articulos = soup.find_all('div', class_=lambda x: x and 'image_post' in x.split() and clase_buscar in x.split())
            
            if not articulos:
                logger.warning(f"No se encontraron artículos con clase '{clase_buscar}' en {self.url}")
                # Mostrar las primeras clases encontradas para debug
                all_divs = soup.find_all('div', class_=True)[:5]
                logger.debug(f"Ejemplo de clases encontradas: {[d.get('class') for d in all_divs]}")
                return []
            
            logger.info(f"Encontrados {len(articulos)} artículos con clase '{clase_buscar}'")
    ...
    def _extract_article_data(self, article_div) -> Optional[Dict]:
        ...
        if date_tag:
            data['published_at'] = date_tag.get('datetime', date_tag.get_text(strip=True))
