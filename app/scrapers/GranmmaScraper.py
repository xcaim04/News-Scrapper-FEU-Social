...
class GranmaScraper(BaseScraper):
    ...
    def __init__(self, categoria: str):
        if categoria not in GranmmaData:
            raise ValueError(
                f"Categoria '{categoria}' no encontrada. Opciones: {list(GranmmaData.keys())}"
            )
    ...
    def scrape(self) -> List[Dict]:
        try:
            ...
            if not articulos:
                logger.warning(
                    f"No se encontraron articulos con clase '{self.clase_css}' en {self.url}"
                )
                return []
    ...
    def _extract_article_data(self, article_soup) -> Optional[Dict]:
        ...
        if title_tag:
            ...
            if href:
                data["url"] = (
                    href if href.startswith("http") else f"https://www.granma.cu{href}"
                )
        ...
        if img_tag and img_tag.get("src"):
            src = img_tag["src"]
            data["image_url"] = (
                src if src.startswith("http") else f"https://www.granma.cu{src}"
            )
        ...
        if date_tag:
            data["published_at"] = date_tag.get("datetime") or date_tag.get_text(
                strip=True
            )
        else:
