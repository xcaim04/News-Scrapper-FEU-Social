...
class BaseScraper(ABC):
    ...
    def fetch_page(self, headers: Optional[Dict] = None) -> Optional[requests.Response]:
        ...
        try:
            ...
            logger.info(f"Página obtenida exitosamente: {self.url} (Status: {response.status_code})")
            return response
            
        except requests.exceptions.Timeout:
            logger.error(f"Timeout al conectar con {self.url} después de {self.timeout} segundos")
    ...
    def fetch_with_retry(self, max_retries: Optional[int] = None) -> Optional[requests.Response]:
        """
        ...
        for attempt in range(retries + 1):
            ...
            if attempt < retries:
                ...
                logger.warning(f"Reintento {attempt + 1}/{retries} en {wait_time} segundos...")
                import time
